...
object FileAttributes2 {
  case object NonExistent extends FileAttributes2 {
  ...
  private final class FileAttributesImpl(override val isDirectory: Boolean,
    ...
    override def toString: String =
      s"FileAttributes2(isDirectory = $isDirectory, isOther = $isOther," +
        s"isRegularFile = $isRegularFile, isSymbolicLink = $isSymbolicLink)"
  }
  def apply(path: NioPath): Either[IOException, FileAttributes2] = apply(path, followLinks = true)
  def apply(path: NioPath, followLinks: Boolean): Either[IOException, FileAttributes2] =
    try {
      val attrs =
  ...
  def apply(isDirectory: Boolean,
            ...
            isSymbolicLink: Boolean): FileAttributes2 =
