    ...
    .withTemporaryDirectory { dir =>
      ...
      try {
        assert(watchTest(monitor, AllPass, excludes(subDir.toPath)) {
          IO.createDirectory(subDir)
          IO.setModifiedTimeOrFalse(source, System.currentTimeMillis - 100000)
          ()
        })

        assert(watchTest(monitor, AllPass, excludes(subDir.toPath)) {
          IO.delete(subDir)
          IO.setModifiedTimeOrFalse(source, System.currentTimeMillis + 200000)
          ()
        })
      } finally {
        monitor.close()
  ...
  it should "detect deletion of a directory containing watched files" in IO
    .withTemporaryDirectory { dir =>
      ...
      try {
        ...
        val triggered = watchTest(monitor, AllPass, includesOnly(src.toPath)) {
          IO.delete(subDir)
        }
