...
private[sbt] object CustomFileAttributes {
  def get[T](path: NioPath,
             f: (NioPath, BasicFileAttributes) => Try[T]): Try[CustomFileAttributes[T]] =
    Try {
      val attrs = Files.readAttributes(path, classOf[BasicFileAttributes])
      new Impl(f(path, attrs), true, attrs)
    }
  private[sbt] def get[T](path: NioPath,
                          simpleFileAttributes: SimpleFileAttributes,
                          t: Try[T]): CustomFileAttributes[T] = {
    val attrs = new LazyFileAttributes(path, Some(simpleFileAttributes))
    new Impl(t, simpleFileAttributes.exists, attrs)
  }
  private[sbt] def get[T](path: NioPath,
                          simpleFileAttributes: SimpleFileAttributes,
                          f: (NioPath, SimpleFileAttributes) => Try[T]): CustomFileAttributes[T] = {
    val attrs = new LazyFileAttributes(path, Some(simpleFileAttributes))
    new Impl(f(path, attrs), simpleFileAttributes.exists, attrs)
  }
  private class Impl[T](override val value: Try[T],
